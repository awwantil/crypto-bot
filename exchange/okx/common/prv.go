...
func (prv *Prv) GetOrderInfo(req model.BaseOrderRequest, opt ...model.OptionParameter) (*model.OrderInfoResponse, []byte, error) {
	...
	order, err := prv.UnmarshalOpts.GetOrderInfoResponseUnmarshaler(data)
	if err != nil {
		return nil, responseBody, err
	}

	return order, responseBody, nil
}

func (prv *Prv) GetPendingOrders(pair model.CurrencyPair, opt ...model.OptionParameter) ([]model.OrderInfoResponse, []byte, error) {
...
func (prv *Prv) GetHistoryOrders(pair model.CurrencyPair, opt ...model.OptionParameter) ([]model.OrderInfoResponse, []byte, error) {
...
func (prv *Prv) GetSpotHistoryOrders(opt ...model.OptionParameter) ([]model.OrderInfoResponse, []byte, error) {
	reqUrl := fmt.Sprintf("%s%s", prv.UriOpts.Endpoint, prv.UriOpts.GetHistoryOrdersUri)
	params := url.Values{}
...
func (prv *Prv) CancelOrder(req *model.BaseOrderRequest, opt ...model.OptionParameter) (model.CancelOrderResponse, []byte, error) {
	...
	if err != nil {
		logger.Errorf("[CancelOrder] err=%s, response=%s", err.Error(), string(responseBody))
...
func (prv *Prv) ClosePositions(req *model.ClosePositionsRequest, opt ...model.OptionParameter) (model.ClosePositionResponse, []byte, error) {
	reqUrl := fmt.Sprintf("%s%s", prv.UriOpts.Endpoint, prv.UriOpts.ClosePositionsUri)
	params := url.Values{}
	params.Set("instId", req.InstId)
	...
	if err != nil {
		logger.Errorf("[ClosePositions] err=%s, response=%s", err.Error(), string(responseBody))
...
func (prv *Prv) AmendOrder(req model.AmendOrderRequest, opt ...model.OptionParameter) (model.AmendOrderResponse, []byte, error) {
	...
	details, err := prv.UnmarshalOpts.AmendOrderResponseUnmarshaler(data)
	logger.Info("details", details)
